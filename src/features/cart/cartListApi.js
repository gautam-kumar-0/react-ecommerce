export function fetchCart(user) {
	return new Promise(async (resolve, reject) => {
		const response = await fetch("http://localhost:4000/cart?user=" + user);
...
export function addToCart(item) {
	return new Promise(async (resolve, reject) => {
		const find = await fetch(
			`http://localhost:4000/cart?user=${item.user}&&product=${item.product}`
		);
		const found = await find.json();

		let response;
		if (found.length) {
			response = await fetch(`http://localhost:4000/cart/${found[0].id}`, {
				method: "PATCH",
				body: JSON.stringify({...found[0], quantity: found[0].quantity + 1}),
				headers: {"content-type": "application/json"},
			});
		} else {
			response = await fetch("http://localhost:4000/cart", {
				method: "POST",
		...
		if (found.length) {
			if (found[0].quantity > 1) {
				const response = await fetch(
					`http://localhost:4000/cart/${found[0].id}`,
					{
						method: "PATCH",
						body: JSON.stringify({
							...found[0],
